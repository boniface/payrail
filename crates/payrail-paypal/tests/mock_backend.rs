...
use payrail_paypal::{PayPalConfig, PayPalConnector};
...
async fn paypal_mock_backend_covers_order_status_capture_refund_and_webhook() {
    ...
    let create = payrail_core::CreatePaymentRequest::builder()
    ...
    assert_eq!(status.status, payrail_core::PaymentStatus::Authorized);
    assert_eq!(capture.status, payrail_core::PaymentStatus::Succeeded);
    assert!(matches!(
        refund,
        Err(payrail_core::PaymentError::UnsupportedOperation(_))
    ));
