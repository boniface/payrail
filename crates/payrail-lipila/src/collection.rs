use payrail_core::{CreatePaymentRequest, PaymentError};
use serde_json::Number;

use crate::models::LipilaMobileMoneyCollectionRequest;
...
pub(crate) fn collection_request(
    ...
    let phone = match request.payment_method() {
        payrail_core::PaymentMethod::MobileMoney(method) => &method.phone_number,
        payrail_core::PaymentMethod::Card(_)
        | payrail_core::PaymentMethod::Crypto(_)
        | payrail_core::PaymentMethod::Stablecoin(_)
        | payrail_core::PaymentMethod::PayPal(_) => {
        ...
        _ => {
            return Err(PaymentError::UnsupportedPaymentMethod(
                "lipila does not support this payment method".to_owned(),
            ));
        }
    };
