...
- Stripe custom Checkout returns a Checkout Session client secret for Stripe.js initialization;
  provider connectors do not log this value.
- Provider metadata is sent to payment providers and can be visible in provider dashboards and
  reports. PayRail documentation requires callers to keep secrets, credentials, card data, bank
  data, and sensitive personal data out of metadata.
...
| `payrail/src/providers/stripe/client.rs` | 93.42% | Hosted Checkout, custom Checkout, idempotent refund, session-to-payment-intent refund resolution, status, and webhook flows are mock-tested; add provider-error tests before stable release. |
